...
namespace cn.com.farsight.SH.SHDbVisit
{
    public class DataManager
    {
        ...
        public bool getModel(NodeData model)
        {
            ...
            sql.Append("select * from data_table ");
        ...
        public bool addModel(NodeData model)
        {
            ...
            strSql.Append("insert into data_table(type,addr,modle,data,ip,battery,checksum,time) values (@type,@addr,@modle,@data,@ip,@battery,@checksum,@time)");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("delete from data_table ");
            string where = string.Empty;
            DbParameter[] parameters = CreateWhere(model, out where);
            if (!string.IsNullOrEmpty(where))
            {
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select count(*) FROM data_table ");
            string where = string.Empty;
            DbParameter[] parameters = CreateWhere(model, out where);
            if (!string.IsNullOrEmpty(where))
            {
        ...
        /// NewOrOld:1为最新，0为最旧
        /// </summary>
        public NodeData GetListByTime(NodeData model,bool NewOrOld)
        {
            StringBuilder sql = new StringBuilder();
            List<NodeData> ModelList = new List<NodeData>();
            ...
            sql.Append(")AS Row, T.*  from data_table T ");
            if (!string.IsNullOrEmpty(where))
            {
                sql.Append(" where " + where);
            }
