using cn.com.farsight.SH.DB.SqlDB;
...
namespace cn.com.farsight.SH.SHDbVisit
{
    public class StatusManager
    {
        DBHelper dbHelper = DBHelper.getHelper();
        public bool getModel(NodeStatus model)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select * from status_table ");
        ...
        public bool addModel(NodeStatus model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("insert into status_table(type,addr,modle,data,ip,battery,checksum,time) values (@type,@addr,@modle,@data,@ip,@battery,@checksum,@time)");
        ...
        public bool deleteModel(NodeStatus model)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("delete from status_table ");
            string where = string.Empty;
            DbParameter[] parameters = CreateWhere(model, out where);
            if (!string.IsNullOrEmpty(where))
            {
        ...
        public List<NodeStatus> getModelList(NodeStatus model)
        {
		...
		/// </summary>
		/// <returns></returns>
        public List<NodeStatus> getModelListAll(NodeStatus model)
        {
            List<NodeStatus> ModelList = new List<NodeStatus>();
            StringBuilder sql = new StringBuilder();
            sql.Append("select ");
        ...
        /// 获取记录总数
        /// </summary>
        public int GetRecordCount(NodeStatus model)
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select count(*) FROM status_table ");
            string where = string.Empty;
            DbParameter[] parameters = CreateWhere(model, out where);
            if (!string.IsNullOrEmpty(where))
            {
        ...
        /// 获取数据列表
        /// </summary>
        public NodeStatus GetListByPage(NodeStatus model)
        {
            StringBuilder sql = new StringBuilder();
            List<NodeStatus> ModelList = new List<NodeStatus>();
            ...
            sql.Append(")AS Row, T.*  from status_table T ");
            if (!string.IsNullOrEmpty(where))
            {
                sql.Append(" where " + where);
            }
        ...
        public bool updateModel(NodeStatus model)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("update status_table set ");
