...
namespace cn.com.farsight.SH.SHDbVisit
{
    public class CommandManager
    {
        ...
        public bool getModel(NodeCommand model)
        {
            ...
            sql.Append("select * from command_table ");
        ...
        public bool addModel(NodeCommand model)
        {
            ...
            strSql.Append("insert into command_table(identify,type,addr,modle,data,userid,time) values (@identify,@type,@addr,@modle,@data,@userid,@time)");
        ...
        public bool deleteModel(NodeCommand model)
        {
            ...
            StringBuilder sql = new StringBuilder();
            sql.Append("delete from command_table ");
            string where = string.Empty;
            DbParameter[] parameters = CreateWhere(model, out where);
            if (!string.IsNullOrEmpty(where))
            {
		...
		/// </summary>
		/// <returns></returns>
        public List<NodeCommand> getModelListTop(NodeCommand model, int top)
        {
            List<NodeCommand> ModelList = new List<NodeCommand>();
            StringBuilder sql = new StringBuilder();
            sql.Append("select ");
        ...
        {
            StringBuilder sql = new StringBuilder();
            sql.Append("select count(*) FROM command_table ");
            string where = string.Empty;
            DbParameter[] parameters = CreateWhere(model, out where);
            if (!string.IsNullOrEmpty(where))
            {
        ...
        public NodeCommand GetListByPage(NodeCommand model)
        {
            ...
            sql.Append(")AS Row, T.*  from command_table T ");
            if (!string.IsNullOrEmpty(where))
            {
                sql.Append(" where " + where);
            }
